...
-- 用户归属字段与身份映射说明（重要）
-- 1) `user_id` 存储的是“内部用户 ID”（后端返回的 user.id），不是用户名。
--    - 在 Mock API 中为递增 ID（u_1、u_2、...）；在正式后端通常为 UUID。
--    - 示例映射：username=123456 -> id=u_1。
-- 2) 共享/公开数据的 `user_id` 应为 NULL。
-- 3) 前端过滤请使用内部 ID 与 NULL 组合（个人+共享），避免用用户名过滤导致看不到数据。
-- 4) 详见 docs/User_Identity_Mapping.md。